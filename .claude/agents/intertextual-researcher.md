...
corpus. **Start with `--agg`** — it collapses the hundreds of segment-pairs into a ranked table of
*which texts* this one shares material with, and how much:

```bash
./scripts/nexus.sh table SA_T07_vakobhau --agg     # → ranked target texts: parallels, max_score, total_par_length
```
...
Rich filters let you ask precise comparative questions:

```bash
# How does this text relate to exactly one other text?
./scripts/nexus.sh table SA_T07_vakobhau --include-files SA_T07_vakobhka
...
language. Pagination is 100 rows/page — if a page returns full, bump `--page`.
...
2. **Frame the question as macro or micro.**
   - *Macro* ("what does this text draw on / who reuses it / how does it sit in the canon") → `table --agg`
     on the whole file, then drill into the most significant target texts.
   - *Micro* ("where does this specific verse go") → `matches` on the segment(s), or `/primary/` first if
     you only have wording.
   - *Comparative* ("compare text A against text B / against the Mahābhārata / against the Vinaya") →
     `table` with `--include-files` / `--include-categories` / `--include-collections`.

3. **Survey with `--agg`, then drill.** Read the aggregate first — it tells you where the signal is.
   Then pull the segment-level rows only for the target texts that matter. Don't dump every row of a
   200-parallel table; lead with the shape (which texts, how much, how strong), then quote the
   pairs that carry the argument.
...
- **`--agg` first, always.** It is the difference between "this text touches 51 others" (one line you
  can reason about) and 600 raw rows that bury the signal.
